...
#[cfg_attr(debug_assertions,
    utoipa::path(
        ...
        responses(
            (status = 200, description = "蔵書の登録に成功した場合。"),
            (status = 400, description = "リクエストのパラメーターに不備があった場合。"),
...
#[cfg_attr(debug_assertions,
    utoipa::path(
        ...
        params(
            ("limit" = i64, Query, description = "一度に取得する蔵書数の上限値の設定"),
...
#[cfg_attr(debug_assertions,
    utoipa::path(
        ...
        path="api/v1/books/{book_id}",
        responses(
            (status = 200, description = "蔵書の削除に成功した場合。"),
            (status = 400, description = "リクエストのパラメーターに不備があった場合。"),
            (status = 404, description = "変更対象の書籍が見つからなかった場合。"),
        ),
        params(
